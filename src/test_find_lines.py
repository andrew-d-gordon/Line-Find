...
class TestFindLines(unittest.TestCase):
    def test_null_points(self):
        ...
        output_unique_2, _ = find_max_unique_point_lines(
            point_set, num_points, pt_threshold
        )
    ...
    def test_three_points(self):
        ...
        valid_line_set = [
            (-1.0, 1.0, 0.0)
        ]  # Only one line so outputs of find_unique and find_max_unique same
        num_points = len(test_points)
    ...
    def test_nine_points(self):
        ...
        valid_line_set_strict = [(-1.0, 1.0, 0.0), (-1.0, 1.0, 3.0), (-1.0, 1.0, 6.0)]
    ...
    def test_float_values(self):
        ...
        self.assertRaises(
            TypeError, find_unique_lines(point_set_1, num_points, pt_threshold), False
        )
        self.assertRaises(
            TypeError,
            find_max_unique_point_lines(point_set_1, num_points, pt_threshold),
            False,
        )
        self.assertRaises(
            TypeError, find_unique_lines(point_set_2, num_points, pt_threshold), False
        )
        self.assertRaises(
            TypeError,
            find_max_unique_point_lines(point_set_2, num_points, pt_threshold),
            False,
        )

        # Test for outputs from both line set finding functions (float set 1)
        output_unique_1_float_test1, _ = find_unique_lines(
            point_set_1, num_points, pt_threshold
        )
        self.assertAlmostEqual(output_unique_1_float_test1, valid_line_set_1)
        output_unique_2_float_test1, _ = find_max_unique_point_lines(
            point_set_1, num_points, pt_threshold
        )
        ...
        output_unique_1_float_test2, _ = find_unique_lines(
            point_set_2, num_points, pt_threshold
        )
        self.assertAlmostEqual(output_unique_1_float_test2, valid_line_set_2)
        output_unique_2_float_test2, _ = find_max_unique_point_lines(
            point_set_2, num_points, pt_threshold
        )
    ...
    def test_higher_volumes(self):
        ...
        num_points_1 = (
            point_set_1.size
        )  # 100 points in bounds (-100,-100) to (100, 100)
        num_points_2 = (
            point_set_2.size
        )  # 1000 points in bounds (-1000,-1000) to (1000, 1000)
        num_points_3 = (
            point_set_3.size
        )  # 1000 points in bounds (-100,-100) to (100, 100)
        pt_threshold = 3
    ...
    def test_higher_threshold(self):
        ...
        output_unique_1_test1, _ = find_unique_lines(
            point_set_1, num_points_1, pt_threshold
        )
        self.assertAlmostEqual(output_unique_1_test1, valid_line_set_1)
        output_unique_1_strict_test1, _ = find_max_unique_point_lines(
            point_set_1, num_points_1, pt_threshold
        )
        ...
        output_unique_1_test2, _ = find_unique_lines(
            point_set_2, num_points_2, pt_threshold
        )
        self.assertAlmostEqual(output_unique_1_test2, valid_line_set_2)
        output_unique_2_strict_test2, _ = find_max_unique_point_lines(
            point_set_2, num_points_2, pt_threshold
        )
